...
public class Exam_JFileChooser extends JFrame {
	...
	private void createMenu() {
		...
		JMenu menu = new JMenu("File");	 // File 메뉴 생성 
		JMenuItem item = new JMenuItem("Open");  // Open 메뉴아이템 생성 
		// File
		// -> Open
		item.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int chosen = 
				chooser.showOpenDialog(bar);  // 파일 선택창을 띄움 
				...
				if(chosen != JFileChooser.APPROVE_OPTION) {
					...
					return;  // 밑에 있는 코드를 실행하면 NullPointException이 발생하기 때문에 종료해줌.
				}
